...
const config: TemplateConfiguration = {
  ...
  colorScheme: "light",
  customerName: "Johnson Controls, Inc.",
  customerUrl: "www.johnsoncontrols.com",
  footerBackgroundColor: "#F4F5F5",
  iconAlt: "72",
  iconUrl: "https://intellicomfort.kraftful.app/app-icon-256.png",
  textColor: "#181718",
};

const TEMPLATE_ID = "jci_ic_kraftful_universal";
...
export default {
  signIn: (userEmail: string): Email => {
    ...
    return {
      ...
      ${button({
        label: "Sign in",
        url: `https://intellicomfort.kraftful.app/signIn/${userEmail.trim()}/${token}`,
        config,
      })}
      ${spacer()}
      ${textRow({
